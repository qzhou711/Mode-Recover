...
ROOT=logs/avoiding/bmd_velocity250/native_trajectory_mmd
LANE="$ROOT/$TAG"
mkdir -p "$LANE/model"
echo "TRAINING weight=$WEIGHT $(date --iso-8601=seconds)" > "$LANE/STATUS"
CUDA_VISIBLE_DEVICES="$GPU" "$PY" -u train_teacher_generated_flow_v2.py \
  ...
  --ctm-ground-truth-mode-balanced \
  --ctm-epochs 500 --ctm-dsm-weight 0.1 \
  --ctm-trajectory-mmd-weight "$WEIGHT" --ctm-trajectory-horizon 4 \
