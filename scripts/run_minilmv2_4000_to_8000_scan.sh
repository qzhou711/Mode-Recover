...
SOURCE=logs/avoiding/teacher_generated_minilmv2_4000_scan/epoch_4000/model/eval_best_flow.pth
ROOT=logs/avoiding/teacher_generated_minilmv2_4000_to_8000_scan
MODEL="$ROOT/training/model"
mkdir -p "$MODEL"

if [[ "$MODE" == train ]]; then
  echo "CONTINUATION_TRAINING $(date --iso-8601=seconds)" > "$ROOT/STATUS"
  CUDA_VISIBLE_DEVICES=0 "$PY" -u train_teacher_generated_flow_v2.py \
    ...
    --pretrain-epochs 4000 --learning-rate 3e-5 \
    --save-pretrain-epochs 1000,2000,3000,4000 \
...
LOCAL_EPOCH=$3
GLOBAL_EPOCH=$4
CHECKPOINT="$MODEL/pretrain_epoch_$(printf '%04d' "$LOCAL_EPOCH").pth"
LANE="$ROOT/epoch_$GLOBAL_EPOCH"
mkdir -p "$LANE/model"
...
sleep 2
cp "$CHECKPOINT" "$LANE/model/eval_best_flow.pth"
