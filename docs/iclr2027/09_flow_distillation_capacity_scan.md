...
> **重大纠正（2026-07-26）**：本文原`3×48→3×48`实验实际为Random-init，并非与`4×72→4×72`一致的Teacher-init同规模对照。显式Teacher-init重跑达到92.1%成功率、23/24模式、熵0.868，基本消除原collapse。因此本文关于`3×48`规模、映射不可压缩和“同规模仍collapse”的结论作废；容量扫描也混入Random-init。详见`11_flow_initialization_correction.md`。
