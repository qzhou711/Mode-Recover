...
def main():
 p=argparse.ArgumentParser();p.add_argument('--bundle-dir',type=Path,required=True);p.add_argument('--model-dir',type=Path,required=True);p.add_argument('--layers',type=int,default=3);p.add_argument('--embed-dim',type=int,default=48);p.add_argument('--heads',type=int,default=3);p.add_argument('--ffn-dim',type=int,default=None);p.add_argument('--steps',type=int,default=1);p.add_argument('--inference-mode',choices=['integrate','boundary'],default='integrate');p.add_argument('--n-trajectories',type=int,required=True);p.add_argument('--episode-start',type=int,default=0);p.add_argument('--seed',type=int,default=42);p.add_argument('--output-dir',type=Path,required=True);a=p.parse_args();a.output_dir.mkdir(parents=True,exist_ok=True)
 meta=torch.load(a.bundle_dir/'deployment_metadata.pt',map_location='cpu');scaler=DeploymentScaler(meta,'cuda');config_path=a.model_dir/'operator_config.json';operator_config=json.loads(config_path.read_text()) if config_path.exists() else None;model=build_flow(a.layers,a.embed_dim,a.heads,'cuda',a.steps,a.ffn_dim,operator_config);model.load_state_dict(torch.load(a.model_dir/'eval_best_flow.pth',map_location='cuda'),strict=True);model.min_action=meta['y_bounds_tensor'][0].cuda();model.max_action=meta['y_bounds_tensor'][1].cuda();model.eval();env=ObstacleAvoidanceEnv(render=False);env.start();paths=[];successes=[];modes=[];start=time.monotonic()
 for ep in range(a.n_trajectories):
  ...
  while not done:
   ...
   with torch.no_grad():
    if a.inference_mode=='boundary':
     noise=torch.randn(1,state.shape[1],2,device=state.device,dtype=state.dtype);zero=torch.zeros(1,device=state.device);out=model.boundary_transition(noise,zero,torch.ones_like(zero),state)
    else:out=model.sample(state,steps=a.steps)
 ...
 metrics={'n_trajectories':len(successes),'success_rate':float(successes.mean()),'successful_trajectories':int(successes.sum()),'unique_successful_modes':int(coverage),'normalized_mode_entropy':entropy,'inference_mode':a.inference_mode,'wall_seconds':float(time.monotonic()-start),'uses_original_demonstrations':False}
